...
const Cart = () => {
  const { cart, savedItems, loading, removeFromCart, updateQuantity, saveForLater, moveToCart } = useCart();
  ...
  const isEmpty = (!cart || cart.items.length === 0) && (!savedItems || savedItems.length === 0);
          ...
          <div className="lg:col-span-8 space-y-6">
            {cart && cart.items.length > 0 ? (
                          ...
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex gap-2">
                              <button 
                                ...
                                className="px-4 py-2 rounded-xl bg-slate-50 text-slate-500 hover:bg-primary/5 hover:text-primary transition-all text-[10px] font-black uppercase tracking-widest"
              ...
              <div className="bg-white rounded-[2.5rem] p-12 text-center border-2 border-dashed border-slate-100">
                <p className="text-slate-400 font-bold">Your active cart is empty.</p>
              </div>
            )}
            ...
            {savedItems && savedItems.length > 0 && (
              <div className="mt-20">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h2 className="text-2xl font-bold text-slate-400 uppercase tracking-[0.2em] text-center">Saved for Later</h2>
                  <div className="h-px flex-grow bg-slate-200"></div>
                </div>
                <div className="grid sm:grid-cols-2 gap-6">
                  {savedItems.map((item) => (
                    <motion.div 
                      ...
                      className="bg-white/60 backdrop-blur-sm rounded-[2rem] p-6 border border-slate-100 flex gap-4 items-center group hover:bg-white transition-all shadow-sm hover:shadow-md"
                    >
                      <div className="w-20 h-20 bg-white rounded-2xl border border-slate-100 p-2 overflow-hidden flex-shrink-0">
                        <img src={getImageUrl(item.product_details?.image)} className="w-full h-full object-contain" alt="" />
                      </div>
                      <div className="flex-grow">
                        <h4 className="font-bold text-slate-900 text-sm mb-1">{item.product_details?.name}</h4>
                        <p className="text-primary font-black text-[10px]">₹{item.product_details?.price}</p>
                        <button 
            ...
            >
              ...
              <div className="space-y-6 mb-10">
                <div className="flex justify-between items-center text-slate-400">
                  <span className="text-sm font-medium">Subtotal ({cart?.items?.length || 0} units)</span>
                  <span className="text-white font-bold">₹{cart?.total_price || 0}</span>
                </div>
                <div className="flex justify-between items-center text-slate-400">
                ...
                <div className="pt-6 border-t border-white/10 flex justify-between items-end">
                  ...
                  <span className="text-5xl font-black text-white tracking-tighter">₹{cart?.total_price || 0}</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                 <Link 
                   to="/checkout" 
                   className={`btn-primary w-full py-6 text-xl shadow-2xl shadow-primary/30 group ${(!cart || cart.items.length === 0) ? 'opacity-50 pointer-events-none' : ''}`}
                 >
